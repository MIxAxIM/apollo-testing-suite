...
for version in "${VERSIONS[@]}"; do
    ...
    if ! git worktree add "$WORKTREE_DIR" HEAD &>/dev/null; then
        echo -e "${RED}Error: Failed to create worktree${RESET}"
        exit 1
    fi

    printf "${CYAN}[%s] Updating Apollo dependency to version: %s...${RESET}\n" "$(date '+%Y-%m-%d %H:%M:%S')" "$version"
    pushd "$WORKTREE_DIR" >/dev/null
    if ! go get github.com/Salvionied/apollo@"$version"; then
        echo -e "${RED}Error: Failed to update Apollo dependency to $version${RESET}"
        popd >/dev/null
